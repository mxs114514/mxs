...
**原理图解（相框与卷轴比喻）**：
- **相框 (Container)**：可视窗口，高度固定。
- **卷轴 (Content)**：所有歌词组成的超长列表，在相框后上下移动。
- **scrollTop**：卷轴被卷上去、藏在相框顶端上面的长度。

**计算步骤**：
1.  `currentLine.offsetTop`：卷轴顶端到目标行上边缘的距离。如果直接卷这么多，目标行会挂在相框最顶端。
2.  `- (容器高度 / 2)`：为了让目标行下来，需要少卷半个相框的高度。此时目标行上边缘对齐相框中线。
3.  `+ (行高/2)`：为了让目标行中心对齐相框中线，需要多卷半行字的高度。
