...
[simple.png](./simple.png) 上半部分的 function component 模型示意。
...
**`useRef` 用于创建对组件内值的引用。它常用于访问 DOM 元素已存在的组件，而不触发组件重新渲染。通过调用 `useRef()`，你可以获得一个包含 `.current` 属性的对象，该属性可以随意更改，但不会导致组件更新。常见用途包括获取表单输入、存储计时器或保持之前渲染的值。**

// todo 写两个名词例子，table什么的标签？一句话代码？不然我看不懂

之后的课程中，你会将原始 HTML 元素的引用放在 `useRef` 里。
